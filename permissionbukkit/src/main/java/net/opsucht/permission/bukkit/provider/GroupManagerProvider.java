...
/*
 * PermissionProvider-Implementierung für GroupManager (weltbasiert). Nutzt AnjoPermissionsHandler für Checks
 * und OverloadedWorldHolder für Mutationen. Online-First-Strategie; offline Fallback nutzt Default-Welt.
 */
public final class GroupManagerProvider implements PermissionProvider {
    ...
    public void add(@NotNull UUID uuid, @NotNull String permission) {
        ...
        if (player != null) GroupManager.BukkitPermissions.updatePermissions(player);
    }
    ...
    public @NotNull Set<String> getGroups() {
        if (Bukkit.getWorlds().isEmpty()) return Set.of();
        String worldName = Bukkit.getWorlds().get(0).getName();
        org.anjocaido.groupmanager.dataholder.OverloadedWorldHolder data =
                gm.getWorldsHolder().getWorldData(worldName);
